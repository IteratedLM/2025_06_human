...
function main()
    ...
    tex_lines = [
        ...
        "\\begin{tabular}{|c|" * join(fill("c", length(shapes)), "|") * "|}",
        "\\hline",
        " & " * join(["\\textbf{$s}" for s in shapes], " & ") * " \\\\ \\hline"
    ]
    ...
    for move in moves
        ...
        for shape in shapes
            ...
            nested_tabular = "\\begin{tabular}{@{}c@{}}" * join(coloured_words, " \\\\ ") * "\\end{tabular}"
            push!(row, nested_tabular)
        end
