...
function startExperiment() {
    ...
    rawSeen.forEach(index => {
        const label = labels[index];
        if (!labelToIndices.has(label)) {
            labelToIndices.set(label, []);
        }
        labelToIndices.get(label).push(index);
    });

    labelToIndices.forEach((indices, label) => {
        if (indices.length === 1) {
            seen.push(indices[0]);
        } else {
            const randomIdx = Math.floor(Math.random() * indices.length);
            const keptIndex = indices[randomIdx];
            ...
            console.log(`Label "${label}" appeared ${indices.length} times.`);
            console.log(`Kept index: ${keptIndex}, moved to unseen: [${moved.join(", ")}]`);
        }
    });



    console.log("raw seen: ", rawSeen);
    console.log("raw unseen: ", rawUnseen);
